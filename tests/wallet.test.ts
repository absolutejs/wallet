...
describe("double-entry wallet", () => {
  ...
  test("captures and refunds an external spend with exact replay binding", async () => {
    ...
    const purchase = await wallet.captureExternalSpend({ accountId: "wallet:buyer", clearingAccountId: "platform:clearing", amountCents: 400, effectId: "effect-1", idempotencyKey: "external-capture", paymentRef: "provider-resource-1", provider: "provider.example", reservationId: reservation.id });
    expect((await wallet.captureExternalSpend({ accountId: "wallet:buyer", clearingAccountId: "platform:clearing", amountCents: 400, effectId: "effect-1", idempotencyKey: "external-capture", paymentRef: "provider-resource-1", provider: "provider.example", reservationId: reservation.id })).id).toBe(purchase.id);
    await expect(wallet.captureExternalSpend({ accountId: "wallet:buyer", clearingAccountId: "platform:clearing", amountCents: 400, effectId: "effect-1", idempotencyKey: "external-capture", paymentRef: "changed", provider: "provider.example", reservationId: reservation.id })).rejects.toThrow(/different input/);
    const refund = await wallet.refundExternalSpend({ accountId: "wallet:buyer", clearingAccountId: "platform:clearing", amountCents: 400, effectId: "effect-1", idempotencyKey: "external-refund", originalPurchaseId: purchase.id, paymentRef: "provider-refund-1", provider: "provider.example" });
    expect((await wallet.refundExternalSpend({ accountId: "wallet:buyer", clearingAccountId: "platform:clearing", amountCents: 400, effectId: "effect-1", idempotencyKey: "external-refund", originalPurchaseId: purchase.id, paymentRef: "provider-refund-1", provider: "provider.example" })).id).toBe(refund.id);
    expect((await wallet.snapshot("wallet:buyer"))?.balanceCents).toBe(1_000);
  });
